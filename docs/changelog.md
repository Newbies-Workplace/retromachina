...
1. Before a release, prepend an entry to `releases` with a `version`, short
   Polish `title`, and user-facing `changes` list.
2. Increase the version using stable `major.minor.patch` versions, such as
   ...
   Do not update `apps/web/package.json`, `package-lock.json`, or other package
   metadata; CI supplies `APP_VERSION` from the GitHub Release tag.
